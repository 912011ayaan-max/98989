...
const StudentDashboard = forwardRef<HTMLDivElement, StudentDashboardProps>(({ currentPage }, ref) => {
  ...
  if (currentPage === 'schedule') {
    return (
      <div ref={ref} className="space-y-6">
        <div>
          <h3 className="text-2xl font-display font-bold">My Schedule</h3>
        ...
        <Card className="shadow-xl border-0 overflow-hidden">
          ...
          <CardContent className="p-0 overflow-x-auto">
            <div className="min-w-[800px]">
              <div className="grid grid-cols-6 border-b border-border/50 bg-muted/50">
                <div className="p-4 font-semibold text-center text-muted-foreground">Time</div>
                {['Monday', 'Tuesday', 'Wednesday', 'Thursday', 'Friday'].map(day => (
                  <div key={day} className="p-4 font-semibold text-center text-primary">{day}</div>
                ))}
              </div>
              <div className="divide-y divide-border/50">
                {[
                  { time: '08:00 - 09:00', mon: 'Mathematics', tue: 'Physics', wed: 'Mathematics', thu: 'Physics', fri: 'Mathematics' },
                  { time: '09:00 - 10:00', mon: 'English', tue: 'Chemistry', wed: 'English', thu: 'Chemistry', fri: 'English' },
                  { time: '10:00 - 10:30', break: true, label: 'Morning Break' },
                  { time: '10:30 - 11:30', mon: 'Science', tue: 'Biology', wed: 'Science', thu: 'Biology', fri: 'Science' },
                  { time: '11:30 - 12:30', mon: 'History', tue: 'Geography', wed: 'History', thu: 'Geography', fri: 'History' },
                  { time: '12:30 - 13:30', break: true, label: 'Lunch Break' },
                  { time: '13:30 - 14:30', mon: 'Art', tue: 'PE', wed: 'Computer Science', thu: 'PE', fri: 'Art' },
                ].map((slot, i) => (
                  <div key={i} className={`grid grid-cols-6 ${slot.break ? 'bg-muted/20' : 'hover:bg-muted/10 transition-colors'}`}>
                    <div className="p-4 text-sm font-medium text-muted-foreground flex items-center justify-center border-r border-border/50">
                      {slot.time}
                    </div>
                    {slot.break ? (
                      <div className="col-span-5 p-4 text-center font-medium text-muted-foreground flex items-center justify-center">
                        <div className="px-3 py-1 rounded-full bg-muted text-xs uppercase tracking-wider">{slot.label}</div>
                      </div>
                    ) : (
                      <>
                        {['mon', 'tue', 'wed', 'thu', 'fri'].map((day, d) => (
                          <div key={d} className="p-2 border-r border-border/50 last:border-0">
                            <div className={`h-full p-3 rounded-xl text-center flex flex-col items-center justify-center gap-1 transition-all hover:scale-105 cursor-default ${
                              // @ts-ignore
                              ['Mathematics', 'Science'].includes(slot[day]) ? 'bg-primary/10 text-primary' :
                              // @ts-ignore
                              ['English', 'History', 'Geography'].includes(slot[day]) ? 'bg-secondary/10 text-secondary' :
                              // @ts-ignore
                              ['Physics', 'Chemistry', 'Biology'].includes(slot[day]) ? 'bg-success/10 text-success' :
                              // @ts-ignore
                              ['PE', 'Art'].includes(slot[day]) ? 'bg-destructive/10 text-destructive' :
                              'bg-muted text-muted-foreground'
                            }`}>
                              <span className="font-semibold text-sm">{(slot as any)[day]}</span>
                            </div>
                          </div>
                        ))}
                      </>
